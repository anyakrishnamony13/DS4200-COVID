...
#             val = pivot.iloc[i, j]
#             if pd.notna(val):
#                 text_color = "white" if abs(val) > 0.30 else "black"
#                 ax.text(
#                     j, i, f"{val:.2f}",
#                     ha="center", va="center",
#                     color=text_color, fontsize=9, fontname=FONT
#                 )
#
#     cbar = fig.colorbar(im, ax=ax)
#     cbar.set_label("Avg Change (During - Before)", fontsize=11, fontname=FONT)
#
#     plt.tight_layout()
#     return fig
#
...
#     Before/During = usage tiers
#     Expected After = expected change tiers
#     """
#     records = []
#
#     for mode_code, mode_label in MODE_MAP.items():
...
#         pre["tier"] = pre[pre_col].apply(categorize_pre)
...
#         now["tier"] = now[now_col].apply(categorize_now)
...
#   <p class="subtitle">
#     Before and During show usage tiers for each transportation mode. Expected After shows
#     how respondents expect their use to change relative to before COVID.
#   </p>
#
#   <div class="buttons">
#     <button onclick="updateChart('Before COVID')">Before COVID</button>
#     <button onclick="updateChart('During COVID')">During COVID</button>
#     <button onclick="updateChart('Expected After')">Expected After</button>
#   </div>
#
...
#         tiers.forEach(tier => {{
#           modeMap.get(mode)[tier] = 0;
#         }});
#       }});
#
#       filtered.forEach(d => {{
#         modeMap.get(d.mode)[d.tier] = +d.percent;
#       }});
#
#       return {{
#         data: Array.from(modeMap.values()),
#         tiers: tiers
#       }};
#     }}
#
